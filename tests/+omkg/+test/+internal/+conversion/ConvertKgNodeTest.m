classdef ConvertKgNodeTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testConvertWithEmbeddedNodeError(testCase)
            % An embedded node that fails to convert is reported by the
            % class it was meant to become and the property of the
            % enclosing instance it was meant to fill, not by a type IRI.
            kgNode = struct(...
                'x_id', 'https://kg.ebrains.eu/api/instances/parent-123', ...
                'x_type', {{'https://openminds.ebrains.eu/core/Person'}}, ...
                'https___openminds_ebrains_eu_vocab_givenName', 'John', ...
                'https___openminds_ebrains_eu_vocab_affiliation', ...
                    testCase.createAffiliationKgNode('Test'));
            ...
            try
                ...
                testCase.verifyEqual(ME.message, sprintf(...
                    ['Failed to create embedded instance of type %s for ', ...
                    'property "affiliation" of %s (%s).'], ...
                    testCase.classNameOf("Affiliation"), ...
                    testCase.classNameOf("Person"), kgNode.x_id))
                testCase.verifyNotEmpty(ME.cause, ...
                    'The error from openMINDS should be kept as the cause')
            end
        end

        function testEmbeddedFailureInListNamesTheElement(testCase)
            % When the property holds a list, the message must say which
            % element failed, indexed the way it is in MATLAB. The first
            % affiliation converts, so the failure is the second.
            kgNode = struct(...
                'x_id', 'https://kg.ebrains.eu/api/instances/parent-123', ...
                'x_type', {{'https://openminds.ebrains.eu/core/Person'}}, ...
                'https___openminds_ebrains_eu_vocab_givenName', 'John', ...
                'https___openminds_ebrains_eu_vocab_affiliation', [...
                    testCase.createAffiliationKgNode(struct(...
            ...
            try
                ...
                testCase.verifyEqual(ME.identifier, ...
                    'OMKG:ConvertKGNode:ConversionFailed')
                testCase.verifySubstring(ME.message, 'property "affiliation(2)"', ...
                    'Message should say which element of the list failed')
            end
        end

        function testNestedEmbeddedFailureGivesPropertyPath(testCase)
            % A node embedded inside an embedded node is located by the
            % property path from the enclosing Knowledge Graph instance,
            % since the nodes in between carry no identifier of their own.
            % Here a property value list holds numerical properties, each
            % holding a quantitative value. The second quantitative value
            % has three uncertainty values where the model allows two.
            kgNode = struct(...
                'x_id', 'https://kg.ebrains.eu/api/instances/list-456', ...
                'x_type', {{'https://openminds.ebrains.eu/core/PropertyValueList'}}, ...
                'https___openminds_ebrains_eu_vocab_propertyValuePair', [...
                    testCase.createNumericalPropertyKgNode('temperature', ...
                        testCase.createQuantitativeValueKgNode(21, [0.5 0.5])), ...
                    testCase.createNumericalPropertyKgNode('humidity', ...
                        testCase.createQuantitativeValueKgNode(40, [1 2 3]))]);
            ...
            try
                ...
                testCase.verifyEqual(ME.identifier, ...
                    'OMKG:ConvertKGNode:ConversionFailed')
                testCase.verifyEqual(ME.message, sprintf(...
                    ['Failed to create embedded instance of type %s for property ', ...
                    '"propertyValuePair(2).value" of %s (%s).'], ...
                    testCase.classNameOf("QuantitativeValue"), ...
                    testCase.classNameOf("PropertyValueList"), kgNode.x_id))
            end
        end

        function testTopLevelFailureNamesClassAndIdentifier(testCase)
            % A Knowledge Graph instance that fails to convert is named by
            % its openMINDS class and its identifier.
            kgNode = testCase.createAffiliationKgNode('Test');
            kgNode.x_id = 'https://kg.ebrains.eu/api/instances/affiliation-789';
            ...
            try
                ...
                testCase.verifyEqual(ME.message, sprintf(...
                    ...
                    testCase.classNameOf("Affiliation"), kgNode.x_id))
            end
        end
    end
    ...
    methods (Static, Access = private)
        ...
        function kgNode = createAffiliationKgNode(memberOf)
            % An affiliation whose memberOf is the given value: a link to
            % an organization converts, text such as 'Test' does not.
            kgNode = struct(...
                'x_type', 'https://openminds.ebrains.eu/core/Affiliation', ...
                'https___openminds_ebrains_eu_vocab_memberOf', memberOf);
        end

        function kgNode = createNumericalPropertyKgNode(name, value)
            % A numerical property holding the given quantitative value node
            kgNode = struct(...
                'x_type', 'https://openminds.ebrains.eu/core/NumericalProperty', ...
                'https___openminds_ebrains_eu_vocab_name', name, ...
                'https___openminds_ebrains_eu_vocab_value', value);
        end

        function kgNode = createQuantitativeValueKgNode(value, uncertainty)
            % A quantitative value; the model allows exactly two
            % uncertainty values, so any other number fails to convert.
            kgNode = struct(...
                'x_type', 'https://openminds.ebrains.eu/core/QuantitativeValue', ...
                'https___openminds_ebrains_eu_vocab_value', value, ...
                'https___openminds_ebrains_eu_vocab_uncertainty', uncertainty);
        end

        function className = classNameOf(typeName)
            % The openMINDS class of a type, as the active model defines it
            typeEnum = openminds.enum.Types(char(typeName));
            className = typeEnum.ClassName;
        end
    end
end
