classdef ConvertKgNodeTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testNestedEmbeddedFailureGivesPropertyPath(testCase)
            ...
            % The chain is given directly, one ancestor per level of
            % embedding, as the conversion of the enclosing instance would
            % build it. The message takes only its class from the failing
            % node, so any node that fails to convert will do.
            ancestors = [...
                struct(...
                    'ClassName', testCase.classNameOf("PropertyValueList"), ...
                    'Identifier', "https://kg.ebrains.eu/api/instances/root-456", ...
                    'PropertyName', "propertyValuePair", ...
                    'Index', 3), ...
                struct(...
                    'ClassName', testCase.classNameOf("NumericalProperty"), ...
                    'Identifier', "", ...
                    'PropertyName', "value", ...
                    'Index', [])];

            try
                omkg.internal.conversion.convertKgNode(...
                    testCase.createAffiliationKgNode('Test'), 'Ancestors', ancestors);
                testCase.verifyFail('Should have thrown an error');
            catch ME
                testCase.verifyEqual(ME.identifier, ...
                    'OMKG:ConvertKGNode:ConversionFailed')
                testCase.verifyEqual(ME.message, sprintf(...
                    ...
                    '"propertyValuePair(3).value" of %s (%s).'], ...
                    testCase.classNameOf("Affiliation"), ...
                    testCase.classNameOf("PropertyValueList"), ...
