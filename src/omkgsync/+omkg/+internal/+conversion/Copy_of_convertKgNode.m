...
%       multiple input kgNode structures are provided.
    ...
    for i = 1:numel(propertyNames)
        ...
        if ~isprop(omDummyNode, currentPropertyName)
            n = numel(UNDEFINED_PROPS_TABLE) + 1;
            UNDEFINED_PROPS_TABLE(n).type = class(omDummyNode);
            if isempty(identifier)
                UNDEFINED_PROPS_TABLE(n).identifier = options.ParentNode.x_id;
            else
                UNDEFINED_PROPS_TABLE(n).identifier = identifier;
            end
            UNDEFINED_PROPS_TABLE(n).propName = currentPropertyName;
            UNDEFINED_PROPS_TABLE(n).propValue = currentPropertyValue;
        ...
        elseif ischar(currentPropertyValue)
            convertChar = getpref('omkg', 'ConvertChar');
            if convertChar
